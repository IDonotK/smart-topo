<template>
  <div class="topo-time-line">
    ...
    <div class="mode-picker">
      <vxe-select size="mini" v-model="viewMode.mode">
    ...
    >
      ...
      >
        <div class="range-side side-left">
          <span class="left-time" v-show="isTimeLabelVisible">{{ curPipsTimeLabel[0] }}</span>
        </div>
        <div class="range-side side-right">
          <span class="right-time" v-show="isTimeLabelVisible">{{ curPipsTimeLabel[1] }}</span>
        </div>
      </div>
    </div>
    <!-- 控制器 -->
